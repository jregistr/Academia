...
import com.jeff.chaser.models.components.motion.{VelocityComponent, TransformComponent}
...
abstract class AiStateSystem[T <:AiStateComponent](aiType: Class[T]) extends EntitySystem {
  ...
  private val aiFamily = Family.all(aiType, classOf[TransformComponent], classOf[VelocityComponent]).get()
  ...
  override def update(deltaTime: Float): Unit = {
    entities.foreach(entity => processEntity(entity, deltaTime))
  }
