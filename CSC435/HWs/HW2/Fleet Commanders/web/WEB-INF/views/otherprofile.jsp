...
<%
    String viewingAtr = (String)request.getAttribute(Controller.VIEWING_USER_INFO);
    String viewingCustAtr = (String)request.getAttribute(Controller.VIEWING_USER_CUSTOMIZATION);
...
<%
    ...
    JsonObject viewingObj = parser.parse(viewingAtr).getAsJsonObject();
    int viewID = viewingObj.get(Controller.getUserIdColumn()).getAsInt();
    String  viewUname = viewingObj.get(Controller.getUserNameColumn()).getAsString();

    JsonObject viewCustObj = parser.parse(viewingCustAtr).getAsJsonObject();
    String viewProf = viewCustObj.get(Controller.getProfileImgColumn()).getAsString();
...
<header>
    ...
    <ul class="nav-right">
        ...
        <li>
            <a href="Profile"><%=viewUname%></a>
        </li>
        <li>
            <div style="width: 50px; height: 95%; margin: auto">
                <img class="imgFit" src="<%=viewProf%>">
            </div>
        </li>
        <li>
...
<div id="mainDisplay">
    <div class="row" style="width: 100%; height: 80px; margin: auto; padding: 5px; background-color: #f5f5f5">
        ...
        <h2 class="text-capitalize"><b><%=uName%></b></h2>
    </div>
            ...
            <div role="tabpanel" class="tab-pane myTabBody" id="Battles">
                <%if(battles != null && !battles.isEmpty()){%>
                    <%JsonObject object = parser.parse(battles).getAsJsonObject();
                        JsonArray bArray = (object != null) ? object.getAsJsonArray("battles") : null;
                    %>
                    <%if(bArray != null){%>
                ...
                <div class="col-xs-4">
                    <div id="messages">
                        ...
                        <%JsonObject commentsObj = parser.parse(commentsAtr).getAsJsonObject();
                            JsonArray comments = (commentsObj != null && !commentsObj.isJsonNull()) ? commentsObj.getAsJsonArray(Controller.getCommentsTableName()) : null;
                        %>
                        <%if (comments != null){%>
                    ...
                    <div>
                        <form class="form-group-lg">
                            ...
                            <button value="<%=viewID%>,<%=id%>" id="postComment" class="form-control btn btn-primary" type="button">Post Comment</button>
                        </form>
                    </div>
                </div>
            </div>
